...
export class Setup {
  ...
  async run(progress: (label: string) => void): Promise<Runtime> {
    progress('Checking Vibenet and discovering test contracts…')
    ...
    for (const side of sides) {
      progress(`${side === 'player' ? 'You' : side === 'jev' ? 'Jev' : 'Round pot'}: funding test gas. Faucet cooldowns may take a minute…`)
      await faucet.ensureEth(accounts[side].account.address, MIN_ETH_BOOTSTRAP)
      if (side !== 'pot') {
        progress(`${side === 'player' ? 'You' : 'Jev'}: requesting test USDV…`)
    ...
    for (const side of sides) {
      progress(`Deploying ${side === 'player' ? 'your' : side === 'jev' ? "Jev’s" : 'the pot'} smart account…`)
