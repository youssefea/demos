...
The game stays unmounted behind a full-page loading screen until funding, wallet deployment, and final balance checks succeed. The BTC market subscription and round timer also start only after setup succeeds. Setup events drive five real stages: network checks, account creation/recovery, five funding checks, three deployments/gas calibrations, and final balances. Stage completion is never driven by a cosmetic timer.
