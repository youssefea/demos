...
1. **Landing on the page automatically funds and deploys** three memory-only smart accounts: you, Jev, and the pot. All receive faucet test gas; only you and Jev receive faucet USDV (the current faucet grants 1,000 USDV each; balances are read from the chain, not assumed). Zero-value token self-transfers deploy the accounts without funding the pot. Five sequential faucet drips can take a minute or longer. Retrying setup keeps the same accounts and signed deployment bytes. React rerenders/effect replay reuse the setup promise rather than funding twice.
2. **Tap Up or Down** to place a 1-USDV test bet. There is no setup/start/confirm button in the normal flow and no separate deadline to click. The direction tap calls the real `typesafe-ai/jev` evaluation model through Vercel AI SDK `experimental_evaluate`. The model sees at most 32 recent Coinbase trade prices/timestamps, never your pick or wallet information. There is no scripted fallback.
...
The Playwright smoke **mocks** the price socket, model response, and setup/network boundaries. It verifies automatic setup/retry, one-tap bets, next-round readiness, no idle model/wager loop, payout/refund, provider outage, and compact above-the-fold controls at 1440px/390px. Screenshots: `/tmp/one-second-simple-{width}.png`. It is **not evidence of live model calls or settlement**. The smoke requires Vite dev source routes. Unit tests additionally cover repeated taps, hidden-tab cancellation, stale/gapped prices, partial funding, unknown/late receipts, refunds, net payout, deduplication, and spending caps. `fighter-ai/tests/predict.test.ts` covers schema/CORS/body/method/rate limits, timeout and provider failure.
