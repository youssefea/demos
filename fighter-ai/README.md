...
The request is `{ "version": 1, "ticks": [{ "price": 65000.01, "time": 1234567890000 }, ...] }` (illustrative only; real requests need fresh timestamps). There must be 2–32 strictly time-ordered price observations, finite positive prices no greater than 10 million, safe-integer millisecond exchange observation timestamps within the last 30 seconds and at most 2 seconds in the future (bounded exchange/server clock tolerance shared with the browser), and a latest timestamp no older than 3.5 seconds. Observations include actual trades and coverage-verified exchange heartbeats carrying forward the last traded price; they do not claim that a new trade occurred. The frontend coalesces same-millisecond observations for this price-only API while retaining every ordered trade for its exact cutoff. Extra fields—including human picks, prompts, and wallet addresses—are rejected. The response is `{ "pick": "up", "model": "typesafe-ai/jev", "inferenceMs": 123 }`, with `pick` strictly `up` or `down`. The model does not see the human choice. Feed authenticity is not attested: these are bounded browser-supplied prices for a trusted-client test demo.
